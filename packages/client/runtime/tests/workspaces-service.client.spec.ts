...
describe('WorkspaceRuntime', () => {
  ...
  it('unarchives a session, projects the set from the response, and leaves a failure untouched', async () => {
    const ctx = new Context()
    const api = new FakeApiClient()
    const sessions = new SessionRuntime(ctx, api, fakeRemote())
    const workspaces = new WorkspaceRuntime(ctx, api, sessions)
    ...
    api.onWorkspaceList = () => Promise.resolve(ok({
      items: [], archivedSessionIds: [sid('s-idle'), sid('s-open')],
    }) as never)
    await Promise.all([sessions.refresh(), workspaces.refresh()])
    await Promise.resolve()
    expect(workspaces.list.getSnapshot().archivedSessionIds).toEqual(['s-idle', 's-open'])

    // Unarchiving installs the unary echo: the set shrinks immediately.
    ...
    api.onWorkspaceUnarchiveSession = () => Promise.resolve(err({
      code: 'internal', message: 'unarchive failed', details: {},
    }))
    await expect(workspaces.unarchiveSession(sid('ghost'))).rejects.toThrow(/internal/)
    expect(workspaces.list.getSnapshot().archivedSessionIds).toEqual(['s-open'])
  })
