...
describe('Host Workspace increments', () => {
  ...
  it('unarchives a session from the global set, restoring its accounting slot, and streams the set once', async () => {
    const { api, root } = await harness()
    ...
    const second = SessionId('session-kept-archived')
    ...
    expect(expectOk(await api.workspace.list(request({}))).archivedSessionIds).toEqual([first, second])
    ...
    // Attach prepends, so the later-created session leads the account.
